    ...
    return {
      ...
      batteryState: {
        ...
        health_info: {
          ...
          score: 95,
          rul_percentage: 100,
          estimated_remaining_cycles: 1000,
          estimated_remaining_months: 36,
          cycle_count: 0,
          health_percentage: 100,
          voltage_stability: 0.001,
          internal_resistance: 0.100,
          internal_resistance_trend: 0.0001,
          temperature_stability: 1.0,
          max_temperature: 25,
          avg_temperature: 25,
          charge_efficiency: 0.95,
          usage_pattern: '正常使用',
          maintenance_schedule: '每6个月检查',
          recommendations: ['保持适宜的充电环境温度', '避免深度放电']
        },
        // 新增：充电优化状态字段
    ...
    updateBatteryState(data) {
      ...
      this.batteryState = {
        ...
        health_info: data.health_info ? {
          ...this.batteryState.health_info,
          ...data.health_info
        } : this.batteryState.health_info,
        charging_optimization: data.charging_optimization ? {
          ...this.batteryState.charging_optimization,
          ...data.charging_optimization,
          adjusted_params: data.charging_optimization.adjusted_params ? {
            ...this.batteryState.charging_optimization.adjusted_params,
            ...data.charging_optimization.adjusted_params
          } : this.batteryState.charging_optimization.adjusted_params
        } : this.batteryState.charging_optimization,
        health_warnings: data.health_warnings || this.batteryState.health_warnings || []
      };
      
      // 确保RUL值在合理范围内
      if (this.batteryState.estimated_rul !== undefined) {
        this.batteryState.estimated_rul = Math.min(100, Math.max(0, this.batteryState.estimated_rul));
      }
      
      console.log('电池状态已更新:', {
        rul: this.batteryState.estimated_rul,
        healthInfo: !!this.batteryState.health_info,
        optimization: !!this.batteryState.charging_optimization,
        optimizationEnabled: this.batteryState.charging_optimization?.enabled
      });
    },
