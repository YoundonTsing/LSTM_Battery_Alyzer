...
class CNNLSTM_RULModel:
    ...
    def convert_pack_to_cell_voltage(self, pack_voltage):
        """将整包电压转换为单体电池等效电压
        ...
        返回:
            cell_voltage: 单体电池等效电压 (V)
        """
        cell_count = BATTERY_CONFIG.get("cell_count_series", 100)
        cell_voltage = pack_voltage / cell_count
        logger.debug(f"电压转换: {pack_voltage}V (整包) → {cell_voltage:.3f}V (单体)")
        return cell_voltage
    
    def convert_cell_to_pack_voltage(self, cell_voltage):
        """将单体电池电压转换为整包电压
        ...
        logger.debug(f"电压转换: {cell_voltage:.3f}V (单体) → {pack_voltage}V (整包)")
        return pack_voltage
    ...
    def adjust_charging_parameters(self, battery_state, rul_percentage):
        ...
        pack_voltage = battery_state.get("voltage", 375.0)  # 整包电压 (400V平台)
        current = battery_state.get("current", 0)
        internal_resistance = battery_state.get("internal_resistance", 0.1)
        
        # 将整包电压转换为单体等效电压进行策略判断
        cell_voltage = self.convert_pack_to_cell_voltage(pack_voltage)
        
        # 获取单体电池配置参数
        cell_max_voltage = BATTERY_CONFIG.get("cell_max_voltage", 4.0)
        cell_cc_to_cv_voltage = BATTERY_CONFIG.get("cell_cc_to_cv_voltage", 3.95)
        
        # 默认参数 - 基于400V平台电池规格，但策略基于单体电池逻辑
        default_params = {
            "cc_current": 0.5,  # 恒流充电电流 (C-rate)
            "cv_voltage": self.convert_cell_to_pack_voltage(cell_max_voltage),  # 整包恒压电压
        ...
        # 电压调整因子：基于单体等效电压判断，接近充满时更保守
        voltage_factor = 1.0
        if cell_voltage > 4.1:
            voltage_factor = 0.8  # 接近充满电压 (>410V整包)
        elif cell_voltage > 4.0:
            voltage_factor = 0.9  # 较高电压 (>400V整包)
        ...
        # 应用调整因子 - 策略基于单体电池逻辑，但返回整包参数
        # 计算调整后的单体电压，然后转换为整包电压
        adjusted_cell_cv_voltage = cell_max_voltage * (0.95 + 0.05 * rul_factor) * strategy["cv"]
        adjusted_pack_cv_voltage = self.convert_cell_to_pack_voltage(adjusted_cell_cv_voltage)
        
        adjusted_params = {
            "cc_current": default_params["cc_current"] * combined_factor * strategy["cc"],
            "cv_voltage": adjusted_pack_cv_voltage,  # 整包恒压充电电压
        ...
        # 记录详细的调整过程 - 包含400V平台转换信息
        logger.info(f"充电参数调整详情 (400V平台):")
        logger.info(f"  - 整包电压: {pack_voltage:.1f}V → 单体等效: {cell_voltage:.3f}V")
        ...
        logger.info(f"  - 电压因子: {voltage_factor:.2f} (基于单体{cell_voltage:.3f}V)")
        logger.info(f"  - 综合因子: {combined_factor:.2f}")
        logger.info(f"  - 充电策略: {charging_strategy}")
        logger.info(f"  - 调整后CV电压: {adjusted_pack_cv_voltage:.1f}V (整包)")
